...
async function viewAdmin(rpcClient: ConcordiumGRPCClient, DEX_CONTRACT_INDEX: bigint) {
    const res = await rpcClient.invokeContract({
        method: ReceiveName.fromString(`${CONTRACT_NAME}.view`),
        contract: ContractAddress.create(DEX_CONTRACT_INDEX, CONTRACT_SUB_INDEX),
        parameter: Parameter.empty(),
    });
    if (!res || res.tag === 'failure' || !res.returnValue) {
        throw new Error(
            `RPC call 'invokeContract' on method '${CONTRACT_NAME}.view' of contract '${DEX_CONTRACT_INDEX}' failed`
        );
    }
...
async function updateWCCDBalanceAccount(rpcClient: ConcordiumGRPCClient, account: string, DEX_CONTRACT_INDEX: bigint) {
    ...
    if (!res || res.tag === 'failure' || !res.returnValue) {
        throw new Error(
            `RPC call 'invokeContract' on method '${CONTRACT_NAME}.balanceOf' of contract '${DEX_CONTRACT_INDEX}' failed`
        );
    }
...
interface ConnectionProps {
    ...
    DEXContractIndex: bigint;
}

export default function DEX(props: ConnectionProps) {
    const { walletConnectionProps, DEXContractIndex } = props;
    ...
    useEffect(() => {
        if (grpcClient && connection) {
            viewAdmin(grpcClient, DEXContractIndex)
                .then((a) => {
                    setAdmin(a);
                    setAdminError('');
                })
    ...
    useEffect(() => {
        if (grpcClient && connection && account) {
            updateWCCDBalanceAccount(grpcClient, account, DEXContractIndex)
                .then((a) => {
                    setAmountAccount(a);
            ...
            <div style={contentContainerStyle}>
                <h1>
                    ...
                    {network.name === 'testnet' ? (
                        <p style={{ color: 'white' }}>CONCORDIUM DEX</p>
                    ) : (
                        <p style={{ color: 'red' }}>MAINNET</p>
                    )}
                </h1>

                <h1 className="header">CONVERT any Cis2 token &#8644; CCD</h1>
                <h3>Convert your cis2 token to CCD and vice-versa  on the Concordium {network.name}</h3>
                    ...
                    <label>
                        ...
                        {connection instanceof BrowserWalletConnector && activeConnectorType === BROWSER_WALLET && (
                            <button
                                ...
                                onClick={() => {
                                    if (account) {
                                        addWCCDToWallet(account, connection, [''], DEXContractIndex, CONTRACT_SUB_INDEX);
                                    }
                                }}
                            >
                                Add Token to wallet
